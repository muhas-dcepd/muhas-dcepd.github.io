...
def optimise_site():
 for file,(route,title,description) in PAGES.items():
  ...
  if route=='dcepd-courses/':
   courses=json.loads((ROOT/'dcepd-courses/catalogue.json').read_text())['courses'];page['mainEntity']={'@type':'ItemList','numberOfItems':len(courses),'itemListElement':[{'@type':'ListItem','position':i+1,'item':{'@type':'Course','name':c['title'],'courseCode':c['code'],'url':BASE+route+'#course-'+c['id'],'provider':{'@id':BASE+'#dcepd'}}} for i,c in enumerate(courses)]}
 ...
 (ROOT/'sitemap.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+''.join('<url><loc>'+BASE+r+'</loc></url>' for r,_,_ in PAGES.values())+'</urlset>')
